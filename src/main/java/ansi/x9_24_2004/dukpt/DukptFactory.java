...
public class DukptFactory {
    ...
    public BitArray computeKey(final BitArray bdk, final BitArray ksn, final IfsfKeyMask ifsfKeyMask) {
        final BitArray ipek = getIpek(bdk, ksn);
        final BitArray transactionKey = deriveSessionKey(ipek, ksn);
    ...
    public BitArray computeAnsiX924version2009DataKey(final BitArray bdk,
                                                      final BitArray ksn) {
        final BitArray derivedKey = computeKey(bdk, ksn, IfsfKeyMask.REQUEST_DATA_MASK);
        ...
        return new BitArray(ByteArrayUtils.concat(firstComponent, secondComponent));
    }

    BitArray getIpek(final BitArray key, final BitArray ksn) {
        byte[][] ipek = new byte[2][];
        BitArray keyRegister = key.get(0, key.size());
        BitArray data = ksn.get(0, ksn.size());
        data.clear(59, 80);
        ...
        return new BitArray(ByteArrayUtils.concat(ipek[0], ipek[1]));
    }

    // See: https://github.com/SoftwareVerde/java-dukpt
    BitArray deriveSessionKey(final BitArray ipek, final BitArray ksn) {
        final BitArray counter = ksn.get(0, ksn.size());
        BitArray transactionKey = (BitArray) ipek.clone();
    ...
    // See: https://github.com/SoftwareVerde/java-dukpt
    BitArray generateTransactionKey(final BitArray transactionKey, final BitArray data) {
        BitArray keyReg = transactionKey.get(0, transactionKey.size());
        BitArray reg1 = data.get(0, data.size());
        // step 1: Crypto Register-1 XORed with the right half of the Key Register goes to Crypto Register-2.
        BitArray reg2 = reg1.get(0, 64); // reg2 is being used like a temp here
        ...
        reg2 = new BitArray(des.encrypt(keyReg.get(0, 64), reg2.toByteArray()));
        ...
        reg1 = new BitArray(des.encrypt(keyReg.get(0, 64), reg1.toByteArray()));
        ...
        return new BitArray(ByteArrayUtils.concat(reg1b, reg2b));
    }
